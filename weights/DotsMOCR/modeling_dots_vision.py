...
class VisionSdpaAttention(nn.Module):
    ...
    def forward(
        ...
        # Process each sub-sequence independently via cu_seqlens.
        # This avoids allocating an O(N^2) attention mask and lets SDPA
        # use its fused O(N) memory kernel on each chunk.
        seqlens = (cu_seqlens[1:] - cu_seqlens[:-1]).tolist()
        q_seqs = torch.split(q, seqlens, dim=0)
        k_seqs = torch.split(k, seqlens, dim=0)
        v_seqs = torch.split(v, seqlens, dim=0)

        outputs = []
        for q_i, k_i, v_i in zip(q_seqs, k_seqs, v_seqs):
            # (seq, heads, dim) -> (heads, seq, dim) for SDPA
            out = F.scaled_dot_product_attention(
                q_i.transpose(0, 1), k_i.transpose(0, 1), v_i.transpose(0, 1),
                dropout_p=0.0,
                is_causal=getattr(self.config, "is_causal", False),
            )
...
class DotsVisionTransformer(PreTrainedModel):
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor, bf16=True) -> torch.Tensor:
        if bf16 and hidden_states.device.type != "mps":
